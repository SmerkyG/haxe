...
class ObjectMap<K:{},V> extends flash.utils.Dictionary implements haxe.Constraints.IMap<K,V> {
	...
	public function keys() : Iterator<K> {
		return new ObjectMapKeysIterator<K>(this);
	}

	public function iterator() : Iterator<V> {
		return new ObjectMapValuesIterator<V>(this);
	}
...
private class ObjectMapKeysIterator<K> {
	var h:Dynamic;
	var index : Int;
	var nextIndex : Int;
...
@:allow(haxe.ds.ObjectMap)
private class ObjectMapValuesIterator<T> {
	var h:Dynamic;
	var index : Int;
	var nextIndex : Int;
